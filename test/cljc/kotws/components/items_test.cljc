...
(deftest default-name-test
  (is (= {:foo {:name :foo}
          :bar {:name :specific-name}}
         (sut/default-name {:foo {}
                            :bar {:name :specific-name}}))
...
(deftest default-with-kw-test
  (is (= {:foo {:name :sfoo
         ...
         (sut/default-with-kw {:foo {:name :sfoo}
                               :bar {:name :bar}}
                              :added-kw
                              :name))
...
(deftest default-with-kws-test
  (is (= {:foo {:name :sfoo
         ...
         (sut/default-with-kws {:foo {:name :sfoo}
                                :bar {:name :bar}}
                               [:added-kw [:copied-from-added-kw :added-kw "-suffix"]]))))
