...
(def ^:private site-url "" #_"https://caumond.fr/")
...
(def ^:private external-links-data
  {:apics "https://www.afrscm.fr/762_p_44059/cpim.html"
   ...
   :clojure-programming "https://www.oreilly.com/library/view/clojure-programming/9781449310387/"
   :clean-archi "https://www.oreilly.com/library/view/clean-architecture-a/9780134494272/"
   :clean-code "https://www.oreilly.com/library/view/clean-code-a/9780136083238/"
   :cor-time-lag "https://www.sciencedirect.com/science/article/pii/S0305054806002930"
   :demo-making "https://en.wikipedia.org/wiki/Demoscene"
   :ddd "https://www.oreilly.com/library/view/domain-driven-design-tackling/0321125215/"
   :distribution-network "https://fr.wikipedia.org/wiki/Algorithme_de_Busacker_et_Gowen"
   ...
   :ejor-transport "https://www.sciencedirect.com/science/article/abs/pii/S0377221708004608"
   ...
   :jewelry-ecommerce "https://web.archive.org/web/20060614232036/http:/www.bijouteriecaumond.fr/"
   :linkedin "http://www.linkedin.com/in/anthony-caumond-a365b15/"
   :make-or-buy-optimization "https://www.investopedia.com/terms/m/make-or-buy-decision.asp"
   ...
   :software-purchasing "https://www.strategieslogistique.com/Michelin-ameliore-son-S-OP-avec"
   :storrito "http://storrito.com"
   :upstream-ope
   "https://www.usinenouvelle.com/article/michelin-veut-reduire-ses-frais-generaux-de-500-millions-d-euros-d-ici-2020.N396442"
   :w3-css "https://www.w3schools.com/w3css/default.asp"
   :w3-template "https://www.w3schools.com/w3css/tryw3css_templates_portfolio.htm"
   :web-caumond-archive "https://web.archive.org/web/20060614232036/http:/www.bijouteriecaumond.fr/"
...
(def ^:private image-links-data
  {:apics {:filename "cpim.png"}
   ...
   :anthony {:filename "anthonycaumond.jpg"
...
(def ^:private doc-links-data
  {:caumond-resume {:filename "caumond_resume.pdf"}
   ;; Coming from https://www.canva.com/design/DAGUsL58eT4/GAwxzsYMR4jqdIo1mRJEaA/edit?utm_content=DAGUsL58eT4&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton
...
(def ^:private external-links
  (-> (map (fn [[link-name external-link]] [link-name {:url external-link
                                                       :absolute-url external-link
                                                       :target "blank"
                                                       :type :external-url}])
           external-links-data)
      (core-map/add-ids :name)))



(def ^:private image-links
  (-> image-links-data
      (core-map/add-ids :name)
      (update-vals
       (fn [{:keys [filename]
         ...
         (let [url (str site-url "images/" filename)]
           (assoc image-link :absolute-url (add-site url) :type :image-link :url url))))))

(def ^:private doc-links
  (-> doc-links-data
      (core-map/add-ids :name)
      (update-vals (fn [{:keys [filename]
                         :as doc-link}]
                     (let [url (str "docs/" filename)]
                       (assoc doc-link :type :doc-link :absolute-url (add-site url) :url url))))))


(def ^:private route-links
  (->> spages/pages
       (mapv (fn [{:keys [url name]
                   :as route-link}]
               [name
                (-> route-link
...
(defn image-link [kw] (if (keyword? kw) (image-links kw) kw))
(defn doc-link [kw] (if (keyword? kw) (get doc-links kw) kw))
(defn route-link [kw] (if (keyword? kw) (get route-links kw) kw))

(defn link
  [kw]
  (if (keyword? kw) (get (merge route-links doc-links image-links external-links) kw) kw))

(defn a [kw content] (wlinks/a (link kw) content))
(defn img-meta [kw] (wlinks/img-meta (link kw)))
(defn link-meta [kw] (wlinks/link-meta (link kw)))
