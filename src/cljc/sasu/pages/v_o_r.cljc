...
(def dic
  {:founder {:fr "Recherche opérationnelle"
   ...
   :tissue-tactical-planning-desc {:fr "Michelin - Décider ce qui est acheté, ce qui est produit"
   ...
   :cor-time-lag {:fr "C.O.R. - Timelag"
                  :en "C.O.R. - Timelag"}
   :cor-time-lag-desc {:fr "Résoudre un problème pratique et théoriquement peu traité"
   ...
   :ejor-transport {:en "EJOR - Transportation"
                    :fr "E.J.O.R. - Transports"}
   :ejor-transport-desc
   {:en "Iterative methods, integer linear programming, graph optimization"
    :fr "Méthodes itératives, programmation linéaire en nombres entiers, optimisation de graphe"}
   ...
   :forge-workshop {:fr "Atelier de forge à chaud"
                    :en "Hot forging workshop"}
   :forge-workshop-desc
   {:fr "Eramet - Intégrer les contraintes de cuisson dans l'ordonnancement de la forge"
   ...
   :cpferv {:fr "Chantiers polyvalents"
            :en "Multipurpose construction sites"}
   ...
   :isima {:fr "Ecole d'ingénieur"
           :en "Initial training"}
   :isima-desc {:fr "ISIMA - Simulation, modélisation et optimisation des systèmes industriels"
                :en "ISIMA - Simulation, modelling and optimization of industrial systems"}
   :isima-long-desc
   {:fr
...
(def items
  [{:or-item :hephaistox}
   {:or-item :tissue-tactical-planning}
   {:or-item :drp}
   {:or-item :cor-time-lag}
   {:or-item :ejor-transport}
   {:start-date "01/09/2001"
    :end-date "01/09/2006"
    :company "Université Blaise Pascale"
    :or-item :phd}
   {:img-url :tire
    :or-item :smpp}
   {:or-item :forge-workshop}
   {:img-url :psa
    :or-item :cpferv}
   {:or-item :isima}])
...
(defn v-o-r
  ...
  (let [tr #(get-in dic [% l])
        ...
        or-items (->> items
                      (mapv (fn [{:keys [img-url or-item]}]
                              {:label (tr or-item)
                               :img-url (slinks/image-link (or img-url or-item))
                               :href (slinks/external-link or-item)
                               :desc (tr-s or-item "-desc")
                               :long-desc (tr-s or-item "-long-desc")})))]
    [:<>
     [:h1.text (tr :founder)]
     [:div.text (tr :intro)]
     [:div.text (tr :intro-2)]
     [wvspace/vertical-spacing]
     [wvheadered-list/detailed-list or-items :small]]))
