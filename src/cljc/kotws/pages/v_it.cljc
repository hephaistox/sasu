...
(def items
  [[:hephaistox {:href :hephaistox-gh}]
   [:storrito {}]
   [:plm {}]
   [:event-stock {:img-url :kafka
                  :href :seb}]
   [:archi {:href :togaf}]
   [:make-or-buy-optimization {}]
   [:distribution-network {}]
   [:dss {:img-url :optimization}]
   [:bcoo {}]
   [:jewelry-website {:img-url :web-caumond
                      :href :web-caumond-archive}]
   [:isima {:start "01/09/1998"
            :end "01/09/2001"
            :company "ISIMA"}]
   [:jewelry-ecommerce {}]
   [:demo-making {}]])
...
(defn v-it
  ...
  (let [current-tr (partial tr l)
        cv-items (kvheadered-list/defaulting* items tr klinks/image-link klinks/external-link)]
    [:<>
