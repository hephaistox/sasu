...
(def items
  [[:hephaistox {}]
   [:tissue-tactical-planning {}]
   [:drp {}]
   [:cor-time-lag {}]
   [:ejor-transport {}]
   [:phd {:start-date "01/09/2001"
          :end-date "01/09/2006"
...
(defn v-o-r
  ...
  (let [current-tr (partial tr l)
        founder-steps (kvheadered-list/defaulting* items tr klinks/image-link klinks/external-link)]
    [:<>
     [:h1.text (current-tr :founder)]
     [:div.text (current-tr :intro)]
     [:div.text (current-tr :intro-2)]
