...
(defn c-tech-stack
  ...
  (let [current-tr (partial tr l)
        tech-stack (kvheadered-list/defaulting* items tr klinks/image-link klinks/external-link)
        ts (get tech-stack l)]
    [:<>
     [:h1.text (current-tr :title)]
     [:p.text (current-tr :sub-title)]
     [kvheadered-list/header ts]
     [:p ""]
     [:h1.text (current-tr :details)]
