...
(def items
  [[:hephaistox {:company :sasu-caumond
                 :start "31/10/2021"}]
   [:plm {:company :michelin
          :start "01/01/2019"
          :end "31/10/2021"
...
(defn v-sc
  ...
  (let [current-tr (partial tr l)
        sc-steps (kvheadered-list/defaulting* items tr klinks/image-link klinks/external-link)]
    [:<>
     [:h1.text (current-tr :sc-title)]
     [:div.text (current-tr :intro)]
     [kvspace/vertical-spacing]
     [kvheadered-list/detailed-list (get sc-steps l) :small]
     [:p ""]]))
