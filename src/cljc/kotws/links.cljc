...
(defn image-link "The image `url` matching `kw`" [kw] (if (keyword? kw) (image-links kw) kw))
...
(def route-links
  (-> kpages/pages
      (update-vals (fn [{:keys [url]
                         :as route-link}]
                     (-> route-link
                         (assoc :absolute-url (add-site url)))))))

(defn route-link "The route `url` matching `kw`" [kw] (if (keyword? kw) (get route-links kw) kw))
