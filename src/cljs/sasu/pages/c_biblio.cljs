(ns sasu.pages.c-biblio
  (:require
   [auto-web.components.c-selector :refer [selected]]
   [auto-web.components.v-slider   :as wvslider]
...
(defn c-biblio
  "Display the bibliography, in language `l`. Currently selected book is stored under `kw`.
  `selected`"
  [l kw]
  (let [tr #(get-in dic [% l])
        books
        (->>
          ...
          (mapv (fn [[item-name item]]
                  (-> item
                      (assoc :img-link (slinks/image-link item-name))
                      (assoc :href (slinks/external-link item-name))
                      (assoc :desc (tr (keyword (str (name item-name) "-desc"))))
                      (assoc :details (tr (keyword (str (name item-name) "-details"))))))))]
    [:<>
     [:h1.text (tr :biblio-title)]
     [:p.text (tr :biblio-intro)]
     (wvslider/subtitle-img-long-desc (tr :desc-title)
                                      (tr :desc-detailed-title)
                                      books
                                      (selected kw)
                                      kw)]))
